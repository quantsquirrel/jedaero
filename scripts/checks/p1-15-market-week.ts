// P1-15: AI-4 주간 시황 — 규칙 기반 6축 등락 계산이 성립하는가
...
const weights: Partial<Record<ThemeCode, number>> = {
  KR_LARGE: 15, KR_THEME: 20, US_INDEX: 40, BOND_CASH: 15, GOLD_COMM: 10, DIVIDEND: 0,
};
...
if (week.moves.length !== THEMES.length) {
  console.log(`축 ${week.moves.length}개 (기대 ${THEMES.length}개)`);
  process.exit(1);
}
...
// 비중 0인 축은 기여도도 0이어야 한다 (DIVIDEND)
const div = week.moves.find((m) => m.code === 'DIVIDEND')!;
if (div.contributionPct !== 0) {
  console.log(`비중 0인 축의 기여도가 0이 아님: ${div.contributionPct}`);
  process.exit(1);
}
// 전 축이 완전히 같은 값이면 시황 카드가 무의미하다
if (week.moves.every((m) => m.changePct === week.moves[0].changePct)) {
  console.log('6축 등락이 전부 동일 — 시드 분산을 확인할 것');
  process.exit(1);
}
