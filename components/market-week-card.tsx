...
type ShownBriefing = { summary: string; questions: string[] };
...
export function MarketWeekCard({
  ...
  moves: ThemeMove[];
  weightedPct: number;
  /** terrain: 시황 + 평일 AI 1문장. full: 내 비중 가중 + 주말 브리핑 */
  variant?: 'terrain' | 'full';
}) {
  const [pending, startTransition] = useTransition();
  const [result, setResult] = useState<{
    briefing?: ShownBriefing;
    source?: 'ai' | 'rule';
    notice?: string;
    error?: string;
  }>({});
  ...
  const run = () =>
    startTransition(async () => {
      const res = variant === 'full' ? await generateBriefingAction() : await generateWeekdayBriefingAction();
