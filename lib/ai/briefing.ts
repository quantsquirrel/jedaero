...
규칙 (어기면 출력이 폐기된다):
- **모든 문장을 존댓말로 끝낸다.** "…였습니다", "…입니다", "…였나요?"
  ★ 이 지시문 자체는 해라체("적는다", "옮긴다")로 쓰여 있지만 그것은 너에게 내리는 지시일
    뿐이다. 네가 «출력하는» 문장은 전부 존댓말이어야 한다. "…였다", "…이다", "…했다"로
    끝나면 폐기된다. 서비스의 다른 모든 화면이 존댓말이고 이 카드만 반말이면 눈에 띈다.
- **입력 필드 이름을 문장에 그대로 옮기지 않는다.** 필드 이름은 밑줄로 이어 붙인 기계용
  이름이라 그대로 쓰면 "내손익을", "등락1위와" 처럼 띄어쓰기가 깨진 한국어가 된다.
  값만 가져와 사람이 읽는 말로 다시 쓴다:
  내손익을_가장_크게_움직인전선 → "내 손익을 가장 크게 움직인 전선"
  내비중퍼센트 → "내 비중", 기여_퍼센트포인트 → "기여" 또는 "%p"
...
출력 예 — **어조와 필드 이름 처리만** 이대로 따른다.
★ 아래 숫자와 전선 이름은 형태를 보이려고 지어낸 것이다. 실제 입력에 없는 값이므로
  그대로 옮겨 적으면 숫자 검증에 걸려 출력이 통째로 폐기된다. 반드시 입력 값으로 바꾼다.
{"summary":"이번 주 가장 오른 전선은 금·원자재 +4.7%였고, 가장 내린 전선은 기타 해외 −3.9%였습니다. 내 손익을 가장 크게 움직인 전선은 기타 해외입니다. 비중 35%에 −3.9%가 겹쳐 −1.4%p였습니다. 가장 크게 움직인 전선과 내 돈을 가장 크게 움직인 전선이 달랐습니다. 내 편성 기준으로는 −1.2%였습니다.","questions":["등락이 가장 컸던 전선과 기여가 가장 컸던 전선은 왜 갈렸나요?","내 손익을 가장 많이 움직인 전선의 비중은 얼마였나요?","이번 주에 아무것도 바꾸지 않는다면 그 이유는 무엇인가요?"]}`;
