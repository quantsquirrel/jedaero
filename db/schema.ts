...
// 논리 스키마 원본: SPEC.md §2. 테이블 12개 (ai_calls·drafts·reviews 포함). 가계부·퀘스트 테이블 없음.
import {
  pgTable,
  uuid,
  text,
...
// 한 줄 회고. 주 1행. 필터 통과분만. 전역 복기용으로만 읽는다. 공유하지 않는다.
// ★ allocations 칸에 넣지 않는다 — 확정 없이 회고한 주도 있다.
export const reviews = pgTable(
  ...
  (t) => [unique('reviews_user_id_week_of_unique').on(t.userId, t.weekOf)],
);
...
export const aiCalls = pgTable('ai_calls', {
  id: uuid('id').primaryKey().defaultRandom(),
  userId: uuid('user_id').notNull(),
  kind: text('kind').notNull(), // AI-3|AI-4|AI-5|AI-7|AI-8|AI-9
