...
//   실선 = 남는다, 점선 = 남지 않는다. 도표에는 신호색(--primary)을 쓰지 않는다 — 데이터는 신호가 아니다 (DESIGN-RULES §1).
...
export function RecordFlow() {
  return (
    <div className="rounded-2xl border border-border bg-card p-6 sm:p-8">
      ...
      >
        <g className="fill-faint text-xs font-semibold" style={{ letterSpacing: '2px' }}>
        ...
        <path d="M530 160 C575 160 570 324 610 324" fill="none" strokeWidth="1.6" className="stroke-[var(--chart-2)]" />
        ...
        <g className="fill-violet-300 text-xs">
        ...
        <g>
          ...
          <text x="16" y="66" className="fill-foreground text-sm font-semibold">별명 · 계급 · 군종</text>
          <text x="16" y="85" className="fill-faint text-xs">입대일 · 전역예정일 · 거리 구간</text>

          <rect x="0" y="112" width="270" height="46" rx="12" className="fill-background stroke-border" />
          <text x="16" y="140" className="fill-foreground text-sm font-semibold">주말 편성 확정 — 판단</text>

          <rect x="0" y="172" width="270" height="46" rx="12" className="fill-background stroke-border" />
          <text x="16" y="200" className="fill-foreground text-sm font-semibold">평일 명령하달 초안</text>

          <rect x="0" y="232" width="270" height="46" rx="12" className="fill-background stroke-border" />
          <text x="16" y="260" className="fill-foreground text-sm font-semibold">그룹 소속 · 분석 동의</text>

          <rect x="0" y="316" width="270" height="46" rx="12" className="fill-background stroke-violet-500/30" />
          <text x="16" y="344" className="fill-foreground text-sm font-semibold">한 줄 회고</text>

          <rect x="0" y="472" width="270" height="52" rx="12" fill="none" strokeDasharray="5 5" className="stroke-input" />
          <text x="16" y="496" className="fill-muted-foreground text-sm font-semibold">실명 · 소속 부대 · 위치</text>
          <text x="16" y="513" className="fill-faint text-xs">애초에 받지 않는 것</text>
        </g>

        {/* 경계 */}
        <g>
          <rect x="340" y="120" width="190" height="80" rx="14" className="fill-secondary stroke-border" />
          <text x="360" y="152" className="fill-foreground text-sm font-bold">규칙</text>
          <text x="360" y="172" className="fill-muted-foreground text-xs">공개된 산식이 계산</text>
          <text x="360" y="188" className="fill-faint text-xs">코드와 화면이 같은 상수</text>

          <rect x="340" y="306" width="190" height="66" rx="14" className="fill-violet-500/10 stroke-violet-500/40" />
          <text x="360" y="334" className="fill-violet-300 text-sm font-bold">AI</text>
          <text x="360" y="354" className="fill-muted-foreground text-xs">읽고 질문 하나를 돌려줌</text>
        </g>

        {/* 남는 곳 */}
        <g>
          <rect x="610" y="40" width="390" height="240" rx="14" className="fill-background stroke-border" />
          <text x="630" y="68" className="fill-foreground text-sm font-bold">저장한다 — PostgreSQL (Neon)</text>
          {STORED.map(([table, note], i) => (
            <g key={table}>
              <text x="630" y={96 + i * 26} className="fill-muted-foreground text-xs">{table}</text>
              <text x="790" y={96 + i * 26} className="fill-faint text-xs">{note}</text>
            </g>
          ))}

          <rect x="610" y="298" width="390" height="104" rx="14" className="fill-[var(--chart-1)]/8 stroke-[var(--chart-1)]/45" />
          <text x="630" y="326" className="fill-[var(--chart-1)] text-sm font-bold">요청 때 계산한다 — 크론 없음</text>
          {COMPUTED.map(([what, how], i) => (
            <g key={what}>
              <text x="630" y={352 + i * 26} className="fill-muted-foreground text-xs">{what}</text>
              <text x="850" y={352 + i * 26} className="fill-faint text-xs">{how}</text>
            </g>
          ))}

          <rect x="610" y="418" width="390" height="56" rx="14" fill="none" strokeDasharray="5 5" className="stroke-violet-500/35" />
          <text x="630" y="444" className="fill-foreground text-sm font-bold">폐기한다</text>
          <text x="630" y="463" className="fill-faint text-xs">생성형 AI가 돌려준 문장 — 호출 여부만 남김</text>

          <rect x="610" y="490" width="390" height="52" rx="14" fill="none" strokeDasharray="5 5" className="stroke-input" />
          <text x="630" y="514" className="fill-muted-foreground text-sm font-bold">입력란 자체가 없다</text>
          <text x="630" y="531" className="fill-faint text-xs">컬럼에도, 입력폼에도, 로그에도 없음</text>
        </g>
      </svg>
      ...
      <div className="mt-6 flex flex-col gap-3 md:hidden">
        ...
        <div className="rounded-xl border border-[var(--chart-1)]/45 bg-[var(--chart-1)]/8 p-4">
          <p className="text-sm font-bold text-[var(--chart-1)]">요청 때 계산한다 — 크론 없음</p>
          <div className="mt-3 flex flex-col gap-2">
